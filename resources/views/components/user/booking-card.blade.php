<div class="col-md-4 m-1 w-72">
    <div class="card">
        ...
        <div class="card-body">
            ...
            @if($booking->maturity)
            <div class="flex justify-between">
                <p class="card-text text-success">Total Profit: <b> {{ $booking->return_amount*$booking->booking_quantity }} </b></p>
            </div>
            @endif
            ...
            <div class="d-grid gap-2 mt-4" style="color: white; background: #017b46; display: grid; gap: 10px; margin-top: 1.5rem;border:1px solid grey;padding:8px; border-radius: 10px;">
                <div class="card-text" style="font-size: 1rem;">You Requested</div>
                <div class="card-text" style="font-size: 1rem;">Withdrawal: <b style="font-weight: bold;">{{ $booking->withdraw }}</b></div>
                <div class="card-text" style="font-size: 1rem;">Re-Invest: <b style="font-weight: bold;">{{ $booking->reinvest }}</b></div>
                <div class="card-text" style="font-size: 1rem;">Profit: <b style="font-weight: bold;">{{ $booking->total_profit }}</b></div>
            </div>

            @else
            <div class="d-grid gap-2 mt-4">
                <a href="{{ url('/mature-batches/request/') }}/{{ $booking->code }}/withdrawal/" class="btn btn-success">Re-Invest Or Withdrawal</a>
            </div>
            @endif
